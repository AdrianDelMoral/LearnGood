...
@section('CSSadded')
    ...
    <link rel="stylesheet" href="{{ URL::asset('css/alumno/subjects.css') }}">
@endsection
...
@section('cuerpo')
    ...
    <div class="container py-5 pb-2">
        ...
        @if ($subjects->count() < 3)
            <a href="{{ route('subjects.create') }}" class="btn btn-success my-3">Crear Plataforma</a>
        @endif
                            ...
                            <div class="fotoPerfil">
                                <div class="cajaImg">
                                    <div class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition">
                                        <img class="h-8 w-8 rounded-full object-cover" src="{{ $subject->subjectImage }}" alt="{{ $subject->role_id }}">
                                    </div>
                                    <div>{{ $subject->subjectImage }}</div>
                                </div>
                            </div></th>
                        <th class="text-center">{{ $subject->nombre }}</th>
                        <th class="text-center">
                ...
                @empty
                    <tr>
                        <th colspan="7" class="text-center">
                            <p class="h4 text-danger fw-bold m-5">No hay Asignaturas con Niveles Creados aun</p>
                        </th>
                    </tr>
                @endforelse
            </tbody>
