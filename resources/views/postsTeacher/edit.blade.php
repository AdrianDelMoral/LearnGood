...
@section('cuerpo')
    <div class="container py-5 text-center">
        ...
        <form action="{{ route('cursosposts.update') }}" method="post" enctype="multipart/form-data">
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" name="entrada" id="entrada" cols="10" rows="2"
                    placeholder="Escriba aqui la entrada principal del Post" required></textarea>
                @error('entrada')
                    <p class="form-text text-danger">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" name="contenidoPost" id="contenidoPost" cols="30" rows="10"
                    placeholder="Escriba aqui el contenido principal del Post" required></textarea>
                @error('contenidoPost')
                    <p class="form-text text-danger">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="container">
        <a href="{{ route('cursosposts.show', $Curso) }}"><button class="btn btn-primary mt-1 mb-5">Volver al
                Listado</button></a>
    </div>
@endsection
